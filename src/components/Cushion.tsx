  ...
  return (
    <svg viewBox="0 0 24 24" className={className} aria-hidden>
      <rect
        ...
        height="13"
        rx="2.5"
        className="fill-amber-400 stroke-amber-700"
        strokeWidth="1"
      />
      <circle cx="5" cy="8" r="1" className="fill-amber-700" />
      <circle cx="19" cy="8" r="1" className="fill-amber-700" />
      <circle cx="5" cy="17" r="1" className="fill-amber-700" />
      <circle cx="19" cy="17" r="1" className="fill-amber-700" />
    </svg>
  );
}
...
export function CushionStack({ count }: { count: number }) {
  ...
  return (
    <span className="inline-flex items-center gap-0.5" title={`${count} zabuton`}>
      ...
      {count === 0 && <span className="text-xs text-zinc-500">0</span>}
    </span>
  );
}
